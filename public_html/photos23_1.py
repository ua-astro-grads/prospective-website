...
imdirs = ['images/gallery_gradlife2023/','images/gallery_steward2023/']
...
def resize_photos(imdir,target_width=500):
    ...
    # Images
    images = [x for x in os.listdir(indir) if (('.png' in x) or ('.jpg' in x) or ('.jpeg' in x) or ('.heic' in x)\
                                               or ('.JPG' in x)  or ('.JPEG' in x) or ('.PNG' in x) or ('.HEIC' in x))]

    images_exist = [x for x in os.listdir(outdir) if (('.png' in x) or ('.jpg' in x) or ('.jpeg' in x) or ('.heic' in x)\
                                                      or ('.JPG' in x)  or ('.JPEG' in x) or ('.PNG' in x) or ('.HEIC' in x))]
...
def fill_photos(imdir,split,heights=[]):
    ...
    # Images
    images = [x for x in os.listdir(imdir) if (('.png' in x) or ('.jpg' in x) or ('.jpeg' in x) or ('.heic' in x)\
                                               or ('.JPG' in x)  or ('.JPEG' in x) or ('.PNG' in x) or ('.HEIC' in x))]
...
#zychen@128.196.208.252/mnt/7ed6eaf8-7c33-40c5-98f9-a19b30f01377/prospective/public_html
os.system("rsync -avr ../public_html/ zychen@128.196.208.252:/mnt/7ed6eaf8-7c33-40c5-98f9-a19b30f01377/prospective/public_html")
